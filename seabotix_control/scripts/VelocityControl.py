...
# Copied from uuv_control_cascaded_pids w/ modifications to
# remove roll/pitch from error terms since seabotix vehicle
# has very little roll/pitch command
...
class VelocityControllerNode:
    ...
    def odometry_callback(self, msg):
        ...
        # Modification here to original VelocityControl.py in uuv_control_cascaded_pids
        # Because the seabotix vehicle has little roll/pitch control authority,
        # it is reliant on restorative forces from being neutrally buoyant to achieve
        # neutral pitch/roll. So controller must ignore roll/pitch in pose error calculation
        # to avoid instability
        e_v_angular[0] = 0.0 # set roll error to 0.0
        e_v_angular[1] = 0.0 # set pitch error to 0.0
    ...
    def config_callback(self, config, level):
        ...
        self.config = config
        # rospy.logwarn("Velocity Control PID: p={}, i={}, d={}, sat={}".format(config['linear_p'], config['linear_i'], config['linear_d'], config['linear_sat']))
